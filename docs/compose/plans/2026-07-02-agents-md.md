...
**Goal:** Add AGENTS.md-style project context loading to nvcoder, following Codex's two-tier model (global + project).

**Architecture:** Create `agent/agents-md.ts` with async discovery that walks global (`~/.nvcoder/AGENTS.md`) and project (Git root → cwd) directories, concatenates files with override precedence, enforces a byte limit, and injects the result into the system prompt at session init.
